/**
 * Stick proof launcher — pi-stick → AgentOS session → terminal tile.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-agentos-stick-proof-"));
  ...
  const shot = join(EVIDENCE_DIR, "V6-00-agentos-stick-live.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-stick missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
