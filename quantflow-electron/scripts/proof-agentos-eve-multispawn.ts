/**
 * Eve AgentOS multi-spawn proof launcher.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-agentos-eve-multispawn-proof-"));
  ...
  const proc = Bun.spawn({
    ...
    env: {
      ...process.env,
      QF_AGENTOS_EVE_MULTISPAWN_PROOF: "1",
  ...
  const shot = join(EVIDENCE_DIR, "V7-01-agentos-eve-multispawn-live.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-eve-multispawn missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
