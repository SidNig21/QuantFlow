/**
 * V6 Eve persona A2A proof launcher.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-eve-a2a-proof-"));
  ...
  const shot = join(EVIDENCE_DIR, "V6-00-eve-a2a.png");
  if (!existsSync(shot)) {
    console.error(`proof:eve-a2a missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
