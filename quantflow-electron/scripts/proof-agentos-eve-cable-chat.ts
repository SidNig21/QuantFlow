/**
 * Eve AgentOS cable-chat proof launcher.
 */
...
async function main(): Promise<void> {
  ...
  const proc = Bun.spawn({
    ...
    env: {
      ...process.env,
      QF_AGENTOS_EVE_CABLE_CHAT_PROOF: "1",
  ...
  const shot = join(EVIDENCE_DIR, "V7-03-agentos-eve-cable-chat-live.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-eve-cable-chat missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
