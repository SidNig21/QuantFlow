...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-agentos-a2a-proof-"));
  ...
  const shot = join(EVIDENCE_DIR, "V6-00-agentos-a2a-live.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-a2a missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
