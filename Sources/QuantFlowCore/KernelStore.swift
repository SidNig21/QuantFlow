...
public final class KernelStore {
    ...
    private func apply(_ command: KernelCommand, commandID: KernelID) throws -> KernelCommandResult {
        ...
        case let .bindWorkerRuntime(workerID, actorID, sessionID, promptable):
            guard let worker = try worker(workerID) else { throw KernelError.missing(workerID) }
            let status: WorkerStatus = promptable ? .active : .idle
            try database.execute("UPDATE worker_instances SET runtime_actor_id = ?, runtime_session_id = ?, status = ?, updated_at = ? WHERE id = ?", [.text(actorID), .text(sessionID), .text(status.rawValue), .integer(now), .text(workerID)])
            try database.execute("UPDATE tiles SET status = ?, updated_at = ? WHERE id = ?", [.text(promptable ? TileStatus.active.rawValue : TileStatus.idle.rawValue), .integer(now), .text(worker.tileID)])
            return try result(commandID, workerID, workflowID: worker.workflowID, taskID: nil, tileID: worker.tileID, kind: "worker.runtime.bound")
    ...
    private func workers(_ workflowID: KernelID) throws -> [WorkerInstance] { try database.query("SELECT id, tile_id, workflow_id, role, harness, model, status, runtime_actor_id, runtime_session_id FROM worker_instances WHERE workflow_id = ? ORDER BY created_at", [.text(workflowID)]) { WorkerInstance(id: database.text($0, 0)!, tileID: database.text($0, 1)!, workflowID: database.text($0, 2)!, role: database.text($0, 3)!, harness: database.text($0, 4)!, model: database.text($0, 5)!, status: WorkerStatus(rawValue: database.text($0, 6)!)!, runtimeActorID: database.text($0, 7), runtimeSessionID: database.text($0, 8)) } }
    private func worker(_ id: KernelID) throws -> WorkerInstance? { try database.query("SELECT id, tile_id, workflow_id, role, harness, model, status, runtime_actor_id, runtime_session_id FROM worker_instances WHERE id = ?", [.text(id)]) { WorkerInstance(id: database.text($0, 0)!, tileID: database.text($0, 1)!, workflowID: database.text($0, 2)!, role: database.text($0, 3)!, harness: database.text($0, 4)!, model: database.text($0, 5)!, status: WorkerStatus(rawValue: database.text($0, 6)!)!, runtimeActorID: database.text($0, 7), runtimeSessionID: database.text($0, 8)) }.first }
    ...
    private func installSchema() throws {
        ...
        CREATE TABLE IF NOT EXISTS worker_instances (id TEXT PRIMARY KEY, tile_id TEXT NOT NULL REFERENCES tiles(id), workflow_id TEXT NOT NULL REFERENCES workflows(id), role TEXT NOT NULL, harness TEXT NOT NULL, model TEXT NOT NULL, status TEXT NOT NULL, runtime_actor_id TEXT, runtime_session_id TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
        ...
        """)
        // M4 migration for M2 databases that already exist in Application Support.
        try? database.execute("ALTER TABLE worker_instances ADD COLUMN runtime_actor_id TEXT")
        try? database.execute("ALTER TABLE worker_instances ADD COLUMN runtime_session_id TEXT")
    }
}
