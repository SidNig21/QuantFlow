...
| R2 — Upstream artifact → downstream context | Scoped · **Codex handoff drafted** (`docs/v4/handoffs/R2-context-envelope.md`) — next to promote | Codex (planned) | Claude | — | Context Envelope v0 (read-path projection) + artifact lineage (`derived_from`). Worker B consumes A's *verified* artifact via the envelope through `harness.send`. **Now lands post-R3/R4:** reuse `task_dependencies kind='context_from'` + `kernel.task.depend`; migration is **006** (004=R3a, 005=R4); composes with the R3 claim gate (which gates `blocks` only). **Product proof bar raised: LIVE canvas, not DevTools.** Pairs with the Track-A *Legend→Run-Workflow context inject* intake (delivery wiring for the envelope). |
