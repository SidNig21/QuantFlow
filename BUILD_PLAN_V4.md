...
| R1 — One real task atom | **Authorized / in progress** (handoff `docs/v4/handoffs/R1-one-real-task-atom.md`) | Codex | Claude | — | Wires the existing `kernel.artifact.create` into the task atom, binds worker via `harness.send`, and replaces `taskVerify`'s rubber-stamp with a structural artifact check. One additive migration (`worker_instances.assigned_task_id`); registers a `mock` harness for CI. **Operator decision — Option B:** also build a **minimal `eve-harness`** (HTTP to local `quantflow-eve`) in this pass; product proof targets the **real Eve worker**, not local-shell (`artifact_root` for Eve = the Eve workspace; verify pulls bytes back through the harness). This follows § Eve Integration R1 delta and is an operator-authorized scope extension beyond the goal-body Direct Repo Scope. |
