...
| R8 — One-click agent/tool onboarding (legend bar) | **Authorized / in progress** (handoff `docs/v4/handoffs/R8-legend-bar-onboarding.md`) | Cursor | Claude | — | **Operator-added** (beyond the territory-map spine). Data-drives `LEGEND_RECIPES` from `role-service.listRoles()` + user-added; "+ Add" form writes `role.json` (CLI) or Eve `manifest.json`; per-recipe R0 readiness badge (green/amber/red); same `legend-spawn → roleId → kernel.worker.spawn` path; makes the R1 `eve-harness` dock-selectable. No Kernel/schema change. Depends on R0 ✅ + R1 ✅. Built by Cursor (Codex rate-limited). |
