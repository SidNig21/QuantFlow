...
| R5 — Human checkpoint / deepen loop | Scoped · **Codex handoff drafted** (`docs/v4/handoffs/R5-human-checkpoint.md`) — next to promote | Codex (planned) | Claude | — | Pausable/resumable run; candidate-set artifact (`kind:'candidate'`); token-bound human selection; deepening tasks spawn. Enforces the decision-authority rule. **Reuses what's shipped:** `checkpoint_state` already exists (R3a — likely NO migration); reuse the 5D `proposalToken` machinery in `conductor-loop.ts` (don't reinvent); `sim` harness (R4) drives `smoke:checkpoint`; `kernel.task.depend` links deepening tasks. The steering wheel, NOT autopilot — run must never auto-decide. Product proof bar: LIVE canvas. |
| R6 — Run templates | Scoped · **Codex handoff drafted** (`docs/v4/handoffs/R6-run-templates.md`) | Codex (planned) | Claude | — | Scout / Research / Deep as saved plan-layer config (NOT a primitive); template **runner = compiler/driver** that compiles to a DAG and reuses `dag-scheduler`(R3)+runtime(R4)+checkpoint(R5) — one executor, not three (F14); attention profile serializes high / parallelizes low. Depends on R5. **Draft caveat: confirm R5's checkpoint API at promotion.** |
| R7 — Judgment & compounding | Scoped · **Codex handoff drafted** (`docs/v4/handoffs/R7-judgment-compounding.md`) | Codex (planned) | Claude | — | Run Replay = **receipt-primary projection** (events not persisted — F1); semantic verify = own stage escalating from R1 structural (not inlined — F15); typed artifacts (additive migration **likely 007**); decision/outcome/lesson → vault; eval **auto-trigger fire-and-forget + non-authoritative** (F16); RL schema-prep only. Depends on R6. |
