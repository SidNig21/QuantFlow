...
| R3 — Small DAG + authority consolidation | **In progress: R3a ✅ · R3b ✅ · R3c pending** (operator-promoted 2026-06-21) | Claude | — | — | **§10.1 resolved → extend Workflow (Workflow IS the run; `run_id ≡ workflow_id`; no `runs` table).** **R3a ✅** migration `004` adds workflow `mode`/`budget_json`/`checkpoint_state`; `queryRun` projection (references only); GLOSSARY + KERNEL_SCHEMA memo. **R3b ✅** `dag-scheduler.ts` (pure `schedulableTasks` + `readSchedulableTasks`) + Kernel claim-gate (`unmetBlockingDependencies`: downstream claimable only when every `blocks` upstream is complete + `verification_passed`) + `kernel.task.depend` command; `smoke:dag` green (collect→{analyze,extract}→synthesize: parallel branches eligible together, synthesize waits for both, Run aggregates ids w/o dup, artifacts carry run_id). Mock harness fixed to allow one worker to run multiple sequential tasks. **R3c (split):** **MCP Kernel reads ✅** — `ipc-kernel-reads.ts` registers read-only `kernel.stateCardList`/`workflowRegion`/`workflowRegionList`/`run`/`evalList` (external-only, F25: Conductor keeps native readonly tools); MCP tools `quantflow_kernel_state_cards`/`_workflow_region`/`_workflow_regions`/`_run`/`_evals` + tests (24 MCP pass). **Envoy consolidation deferred** (operator decision 2026-06-21) — rewiring the LIVE `workflow-service.createWorkflowTask`→`EnvoyTaskService` (competing `envoy_tasks` store) to "Kernel decides; Envoy mirrors" is high blast radius on the daily Run-Workflow flow; do it as a focused, dogfooded pass with `smoke:authority`. |
| R4 — Durable pod runtime | Scoped · **Codex handoff drafted** (`docs/v4/handoffs/R4-durable-pod-runtime.md`) | Codex (planned) | Claude | — | Worker Runtime Manager, budgets on Run (columns already exist from R3a), reload survival, stale/cancel/timeout/recover, simulation harness (`sim` = `mock` + failure catalog), exactly-once commands. Eve lane collapses onto Vercel Workflow durability (driver seams only, no cloud code); local lane = light durability. The heaviest rung. **Builder handoff = `docs/v4/handoffs/R4-durable-pod-runtime.md` (4 sub-milestones R4a–R4d + cumulative guard incl. `smoke:pod`).** Prereq: operator finishes R3c-b (one task authority). |
