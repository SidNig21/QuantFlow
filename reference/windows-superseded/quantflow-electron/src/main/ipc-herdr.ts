 ...
 * ipc-herdr.ts
 *
 * Exposes herdr socket operations to the renderer:
 *
 *   herdr:available  → boolean
 *   herdr:list       → HerdrPane[]
 *   herdr:read       → string   (DEBUG ONLY — not tile display)
 *   herdr:send       → void
 *   herdr:status     → agent_status string (debug/one-shot; badges use events)
 */
...
export function registerHerdrHandlers(): void {
  ...
  ipcMain.handle("herdr:read", (_event, paneId: unknown, lines?: unknown) => {
    ...
    // DEBUG ONLY — pane.read must never become the tile display path.
