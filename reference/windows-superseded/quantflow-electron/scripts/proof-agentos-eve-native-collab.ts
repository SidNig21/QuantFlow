/**
 * Eve AgentOS native cable collaboration proof launcher.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-agentos-eve-native-collab-proof-"));
  ...
  const proc = Bun.spawn({
    ...
    env: {
      ...process.env,
      QF_AGENTOS_EVE_NATIVE_COLLAB_PROOF: "1",
  ...
  const shot = join(EVIDENCE_DIR, "V7-04-agentos-eve-native-collab-live.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-eve-native-collab missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
