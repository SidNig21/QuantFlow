/**
 * V5 orchestrator proof launcher.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-orchestrator-proof-"));
  ...
  const shot = join(EVIDENCE_DIR, "V5-00-orchestrator.png");
  if (!existsSync(shot)) {
    console.error(`proof:orchestrator missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
