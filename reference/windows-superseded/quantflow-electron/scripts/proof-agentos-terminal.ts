/**
 * V1 AgentOS terminal proof launcher.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-agentos-terminal-proof-"));
  ...
  const shot = join(EVIDENCE_DIR, "V1-00-terminal-spawn.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-terminal missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
