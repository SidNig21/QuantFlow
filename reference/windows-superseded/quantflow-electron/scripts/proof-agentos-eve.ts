/**
 * Eve AgentOS proof launcher — proof-only dock recipe → live Eve prompt.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-agentos-eve-proof-"));
  const proc = Bun.spawn({
    cmd: [process.execPath, resolveElectronCli(), "."],
    cwd: ELECTRON_DIR,
    env: {
      ...process.env,
      QF_AGENTOS_EVE_PROOF: "1",
  ...
  const shot = join(EVIDENCE_DIR, "V7-00-agentos-eve-live.png");
  if (!existsSync(shot)) {
    console.error(`proof:agentos-eve missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
