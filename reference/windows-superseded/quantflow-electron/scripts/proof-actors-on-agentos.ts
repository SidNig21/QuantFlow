/**
 * V3 actors-on-agentos proof launcher.
 */
...
async function main(): Promise<void> {
  ensureBuilt();
  const dataRoot = mkdtempSync(join(tmpdir(), "qf-actors-on-agentos-proof-"));
  ...
  const shot = join(EVIDENCE_DIR, "V3-00-actors-on-agentos.png");
  if (!existsSync(shot)) {
    console.error(`proof:actors-on-agentos missing screenshot: ${shot}`);
    process.exit(1);
  }
  process.exit(0);
}
