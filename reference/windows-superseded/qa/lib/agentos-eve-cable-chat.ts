...
function assertEvePromoted(): boolean {
  ...
  if (![...DOCK_SPAWN_ACTOR_IDS].includes('eve')) {
    console.error('agentos-eve-cable-chat: eve missing from DOCK_SPAWN_ACTOR_IDS — U7 promotion regressed');
    return false;
  }
  console.log('agentos-eve-cable-chat: eve is a verified dock spawn actor (U7)');
  return true;
}
